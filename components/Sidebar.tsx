...
const Sidebar: React.FC<SidebarProps> = ({ currentMode, setMode }) => {
  ...
  return (
    <>
      <div className="w-20 md:w-64 bg-black/40 backdrop-blur-xl border-r border-white/10 flex flex-col items-center md:items-stretch py-4 transition-all duration-300">
        <div className="p-6">
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className="hidden md:block font-medium text-sm text-left">{item.label}</span>
              </button>
            );
          })}
        </nav>
        ...
        <div className="p-4 border-t border-base-border relative" ref={userMenuRef}>
          ...
          {isUserMenuOpen && (
            <div className="absolute bottom-full left-4 right-4 mb-2 bg-base-card border border-base-border rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-2 z-50">
              {isAdmin && (
                <button
                  onClick={() => { setMode(AppMode.ADMIN); setIsUserMenuOpen(false); }}
              ...
              <button
                onClick={() => { startTour(currentMode); setIsUserMenuOpen(false); }}
              ...
              <button
                onClick={() => { setMode(AppMode.SETTINGS); setIsUserMenuOpen(false); }}
                className="w-full flex items-center gap-3 px-4 py-3 text-sm text-neutral-300 hover:text-white hover:bg-white/5 transition-colors text-left"
              >
                <Icons.Settings className="w-4 h-4 text-neon" /> Configurações
              </button>
              <button
                onClick={() => { setMode(AppMode.HELP_CENTER); setIsUserMenuOpen(false); }}
